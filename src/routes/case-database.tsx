        ...
        {selectedCase && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-5 backdrop-blur-md" role="dialog" aria-modal="true">
            <div className="relative max-h-[88vh] w-full max-w-4xl overflow-y-auto rounded-2xl border bg-card p-6 shadow-premium">
              ...
              <div className="mt-4 rounded-xl border bg-secondary p-5"><h3 className="text-xl font-black text-foreground">Keyingi qadamlar</h3><div className="mt-3 grid gap-2 md:grid-cols-3">{selectedCase.nextSteps.map((step) => <p key={step} className="rounded-lg border bg-card p-3 text-sm text-muted-foreground">{step}</p>)}</div><div className="mt-4 flex flex-wrap gap-2"><Button variant="premium" asChild><a href={selectedCase.sourceUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" /> Rasmiy manba</a></Button><Button variant="legal" asChild><a href={`https://scholar.google.com/scholar?q=${encodeURIComponent(selectedCase.title)}`} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" /> Google Scholar</a></Button></div><p className="mt-3 text-xs text-muted-foreground">Manba: {selectedCase.source}</p></div>
