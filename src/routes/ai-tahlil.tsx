            ...
            <div className="cinematic-scan rounded-2xl border bg-card p-5 shadow-premium">
              <div className="relative mx-auto aspect-[4/5] max-w-sm overflow-hidden rounded-xl bg-secondary">
                ...
                <div className="portrait-motion absolute inset-x-0 bottom-0 mx-auto w-[94%]">
                  <img src={founderPortrait} alt="CaseMind legal AI vakili portreti" className="relative z-10 w-full object-contain drop-shadow-2xl" loading="eager" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-background/70 via-transparent to-transparent" />
                <div className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full border bg-card/85 px-3 py-2 text-xs font-bold uppercase text-accent backdrop-blur"><ScanFace className="h-4 w-4" /> Live AI avatar</div>
              </div>
            </div>
            <div className="rounded-2xl border bg-card p-6 shadow-premium">
              <Sparkles className="mb-4 h-8 w-8 text-accent" />
              <h2 className="text-4xl font-black text-foreground">Motion portretli AI konsultant</h2>
              <p className="mt-4 leading-8 text-muted-foreground">Yuklangan portret endi sahifada premium motion effekt bilan ko‘rinadi: yengil suzish, mimika signallari va cinematic scan animatsiyasi AI tahlil tajribasini jonlantiradi.</p>
            </div>
          </div>
        </div>
      </section>
